...
% características:
% -Energía.
% -Spectral Flatness Measure (SFM), que indica la planitud espectral de la
% señal.
% -F: representa la componente de la frecuencia más dominante en el
% espectro.
%
...
function [VAD, valores_VAD, E, F, SFM] = VAD(muestra, fs, E_NORMAL, SFM_NORMAL, F_NORMAL)
        ...
        if x < Num_Ventanas-1
            ...
            while y < length(P1)
                if (P1(y,1) == maxP1) && (aux_primera_iteracion == 1)
                    % Busco qué valor de frecuencia corresponde a la posición en la
                    % que se ha encontrado el máximo de P1.
                    F(x,1) = f(1,y);
            ...
            % Me creo un bucle que me compruebe si he analizado las 30 primeras
            % ventas, y si es así, suponiendo que alguna de estas 30 primeras
            % ventanas son silencio, calculo la Min_E, Min_F y Min_SFM.
            if x == 30
                Min_E = min(E);
                Min_F = min(F);
                Min_SFM = min(SFM);
            ...
            contador_temp = 0;
            % Añado 1 al valor del contador si la energía de la ventana menos el
            % mínimo de la energía es mayor o igual que el E_Umbral.
            ...
            % Añado 1 al valor del contador si la SFM de la ventana menos el
            % mínimo de la SFM es mayor o igual que el SFM_Umbral.
            if (SFM(x,1) - Min_SFM) >= SFM_Umbral
                contador_temp = contador_temp+1;
            end
            % Si el contador es mayor que 1, marco la ventana en la que estoy
            % trabajando como ventana con Voz; por el contrario, la marco como
            % Silencio. 
            if contador_temp > 1
                ...
                % comprobaciones.
                contador_voz = 0;
                % Si he marcado la ventana como silencio, recalculo el Min_E.
                Min_E = (((contador_silencio*Min_E)+E(x,1))/(contador_silencio+1));
            end
            % Recalculo el E_Umbral con el nuevo valor de Min_E.
            E_Umbral = E_Umbral_Inicial*log10(Min_E);
            % Me creo dos variables que me indicarán si la ventana en la que estoy
            % se ha marcado como silencio (ventana_silencio = 1) o como voz 
            % (ventana_voz = 1).
            ventana_silencio = 0;
            ventana_voz = 0;
            % Marco la ventana como silencio si hay más de 16 ventanas 
            % sucesivas marcadas como silencio.
            if contador_silencio >= 16
                ventana_silencio = 1;
            end
            ...
            % Si se ha detectado que NO hay voz, se aplica la ganancia.
            if ventana_silencio == 1
                VAD(Inicio_Muestras_Ventana:Fin_Muestras_Ventana,1) = VAD(Inicio_Muestras_Ventana:Fin_Muestras_Ventana,1)*G;
                ...
                x = x+1;
            % Si se ha detectado que SÍ hay voz, simplemente se avanza a la
            % siguiente ventana.
            else
                % En este caso, si se detecta voz ,voy a guardar un 1 como 
                % valor de VAD para esta ventana.
                posicionVAD = Inicio_Muestras_Ventana+(round(Muestras_Ventana/2));
                valores_VAD(posicionVAD,1) = 1;
                % Avanzo el inicio y el fin de las Muestras_Ventana para avanzar a
                % la siguiente ventana, comprobando que si he llegado al final de
                % la señal de voz.
                Inicio_Muestras_Ventana = Fin_Muestras_Ventana;
